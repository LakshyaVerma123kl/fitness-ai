  ...
  const exportPDF = () => {
    try {
      ...
      if (plan.results_timeline) {
        ...
        doc.text(
          `Expected Start: ${plan.results_timeline.estimated_start || "N/A"}`,
          margin,
          yPos
        );
        yPos += 6;
        ...
        if (
          plan.results_timeline.milestones &&
          Array.isArray(plan.results_timeline.milestones)
        ) {
      ...
      if (
        ...
        // Filter out days without exercises and add null checks
        const workoutRows = plan.workout
          .filter(
            (day: any) =>
              day &&
              day.exercises &&
          ...
          .map((day: any) => {
            const exercises = day.exercises
              ...
              .join("\n");
            return [day.day || "Day", day.focus || "General", exercises];
          });

        // Only add table if we have valid rows
      ...
      // Diet Section - WITH FIX
      if (hasDiet && plan.diet) {
        const mealsSource = plan.diet.meals || plan.diet;
        ...
        if (mealsSource) {
          ...
          if (plan.diet.strategy) {
            ...
            if (plan.diet.strategy.week_1) {
              doc.text(
                `Week 1 Focus: ${plan.diet.strategy.week_1}`,
                margin,
                yPos
              );
              yPos += 6;
            ...
            if (plan.diet.strategy.week_2) {
              doc.text(
                `Week 2 Focus: ${plan.diet.strategy.week_2}`,
                margin,
                yPos
              );
              yPos += 10;
          ...
          const dietRows = MEAL_ORDER.filter((key) => mealsSource[key]).map(
            (mealType) => {
              ...
              return [
                ...
                details.meal || "N/A",
                `${details.calories || 0} kcal\nP:${details.protein || 0}g C:${
                  details.carbs || 0
                }g F:${details.fats || 0}g`,
              ];
            }
          );
      ...
      if (shoppingList && shoppingList.length > 0) {
        ...
        const splitText = doc.splitTextToSize(listText, 180);
                  ...
                  ? MEAL_ORDER.map((mealType) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Action Buttons: Recipe + AI Image */}
                          <div className="flex items-center gap-2 w-full">
                            <a

...
export const generateAndDownloadPDF = (
  ...
  try {
    ...
    if (plan.safety_warnings && plan.safety_warnings.length > 0) {
      ...
      doc.text("⚠️ SAFETY WARNINGS", margin + 5, yPos + 7);
    ...
    if (plan.health_considerations) {
      ...
      doc.setFont("helvetica", "bold");
      doc.text("🏥 Health Considerations", margin + 5, yPos + 7);
    ...
    if (plan.hydration || plan.recovery) {
      ...
      doc.setFontSize(14);
      doc.setFont("helvetica", "bold");
      doc.text("💧 Hydration & Recovery", margin, yPos);
      yPos += 8;
      doc.setFontSize(10);
      doc.setFont("helvetica", "normal");
    ...
    if (hasWorkout) {
      ...
      const workoutRows = plan.workout.map((day: any) => {
        const exercises = day.exercises
          .map((ex: any) => {
            let txt = `• ${ex.name} (${ex.sets}x${ex.reps}, Rest: ${ex.rest})`;
    ...
    if (plan.diet && plan.diet.meals) {
      ...
      doc.setFontSize(14);
      doc.setFont("helvetica", "bold");
      doc.text("🍽️ Nutrition Plan", margin, yPos);
      yPos += 8;
    ...
    if (plan.diet && plan.diet.strategy) {
      ...
      doc.setFontSize(12);
      doc.setFont("helvetica", "bold");
      doc.text("📊 Diet Strategy", margin, yPos);
      yPos += 8;
      doc.setFontSize(9);
      doc.setFont("helvetica", "normal");
      ...
      if (plan.diet.strategy.allergy_notes) {
        doc.setFont("helvetica", "bold");
        doc.text("⚠️ Allergy Notes:", margin, yPos);
        yPos += 5;
        doc.setFont("helvetica", "normal");
    ...
    if (plan.supplements && plan.supplements.length > 0) {
      ...
      doc.setFontSize(12);
      doc.setFont("helvetica", "bold");
      doc.text("💊 Recommended Supplements", margin, yPos);
      yPos += 8;
      doc.setFontSize(9);
      doc.setFont("helvetica", "normal");

      plan.supplements.forEach((supp: string) => {
        const suppLines = doc.splitTextToSize(`• ${supp}`, 180);
    ...
    setToast({ show: true, message: "✅ Downloaded!", type: "success" });
  } catch (error) {
    console.error(error);
